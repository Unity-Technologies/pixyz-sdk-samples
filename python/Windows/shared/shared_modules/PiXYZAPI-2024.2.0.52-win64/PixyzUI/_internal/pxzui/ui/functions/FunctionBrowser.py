...
class FunctionBrowser(ConfigOwner):
    ...
    def draw(self):
        if not self.getConfig("visible"):
            return
        _, visible = imgui.begin("Function browser", self.getConfig("visible"))
        ...
        if self.functionList.selectedFunction is not None:
            imgui.text(self.functionList.selectedFunction.module.lower() + "." + self.functionList.selectedFunction.name)
        # function form
