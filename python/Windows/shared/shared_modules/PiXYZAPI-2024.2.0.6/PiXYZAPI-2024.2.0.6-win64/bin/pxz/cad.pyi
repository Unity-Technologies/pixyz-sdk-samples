...
class Bounds1D:
	...
	min: float
	max: float
	def __init__(self, min: float, max: float) -> Bounds1D: ...
	...
...
class Bounds2D:
	...
	u: Bounds1D
	v: Bounds1D
	def __init__(self, u: Bounds1D, v: Bounds1D) -> Bounds2D: ...
	...
...
class EdgeStyle:
	...
	width: float
	pattern: int
	def __init__(self, color: core.ColorAlpha, width: float, pattern: int) -> EdgeStyle: ...
	...

class LegoBrickDefinition:
	"""
	"""
	legoType: LegoType
	length: int
	width: int
	fullHeight: bool
	def __init__(self, legoType: LegoType, length: int, width: int, fullHeight: bool) -> LegoBrickDefinition: ...
	...
...
class OrientedEdge:
	...
	edge: int
	orientation: bool
	def __init__(self, edge: int, orientation: bool) -> OrientedEdge: ...
	...
...
class OrientedFace:
	...
	face: int
	orientation: bool
	def __init__(self, face: int, orientation: bool) -> OrientedFace: ...
	...
...
class SplittedEdge:
	...
	oldEdge: int
	newEdges: list[int]
	def __init__(self, oldEdge: int, newEdges: list[int]) -> SplittedEdge: ...
	...
...
def createRevolveExtrusion(profileCurves: list[int], center: geom.Point3, direction: geom.Point3, angle: float, area: bool) -> int:
	"""create a revolved extrusion

	Args:
		profileCurves (list[int]): Profile curves to revolve
		center (geom.Point3): The center of revolution
		direction (geom.Point3): The profile curves will be revolved in this direction
		angle (float): The revolution will stop at the defined angle
		area (bool): If the profile curves define an area or not
...
def createLegoBrick(brickDefinition: LegoBrickDefinition) -> int:
	"""Creates a BRep Lego brick

	Args:
		brickDefinition (LegoBrickDefinition): The definition of the wanted lego brick
...
def setEdgeStyle(edge: int, style: EdgeStyle) -> None:
	"""Define an edge's edge style

	Args:
		edge (int): The edge to apply to style to
		style (EdgeStyle): The EdgeStyle to apply
	"""
	...
...
def createCoEdge(edge: int, orientation: bool, surface: int = 0, curve2D: int = 0) -> int:
...
def createLoop(coEdges: list[int], check: bool = True) -> int:
...
def invertFaces(faces: list[int]) -> None:
	"""Invert faces
...
def createConeSurface(radius: float, semiAngle: float, matrix: list[list[float]] = geom.IdentityMatrix4) -> int:
	...
	Args:
		...
		semiAngle (float): Semi-angle of the cone
		matrix (list[list[float]]): Positionning matrix of the cone
...
def createCurveExtrusionSurface(generatrixCurve: int, directrixCurve: int, refSurface: int) -> int:
...
def createRevolutionSurface(generatrixCurve: int, axisOrigin: geom.Point3, axisDirection: geom.Point3, startAngle: float = 0, endAngle: float = 360.0) -> int:
	...
	Args:
		...
		startAngle (float): Start angle of the revolution surface
		endAngle (float): End angle of the revolution surface
