...
struct CapsuleParameters {
   ...
   CoreI::Int Latitude;
   CoreI::Int Longitude;
   CapsuleParameters(CoreI::Double _Radius = 250, CoreI::Double _Height = 250, CoreI::Int _Latitude = 16, CoreI::Int _Longitude = 16) : Radius(_Radius), Height(_Height), Latitude(_Latitude), Longitude(_Longitude) {}
};
...
struct LegoReturn {
   ...
   CADI::LegoBrickDefinitionList Bricks;
   LegoReturn(SceneI::Occurrence _Root = 0, CADI::LegoBrickDefinitionList _Bricks = CADI::LegoBrickDefinitionList()) : Root(_Root), Bricks(_Bricks) {}
};

typedef CoreI::List<MapType> MapTypeList;

class MaxTransformationType {
public:
   typedef enum {
   Similarity = 0,
   Affine = 1,
...
private:
   Type _value;
public:
   MaxTransformationType(Type val = Similarity) : _value(val) {}
   MaxTransformationType & operator=(Type val) { _value = val; return *this; }
   MaxTransformationType(const CoreI::Int& val) : _value((Type)val) {}
   MaxTransformationType & operator=(const CoreI::Int& val) { _value = (Type)val; return *this; }
   Type value() const { return _value; }
   operator Type() const { return _value; }
};
