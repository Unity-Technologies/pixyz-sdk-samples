...
typedef struct {
   Algo_BakingMethod bakingMethod;
   Core_Int resolution;
   Core_Int padding;
...
typedef struct {
   ...
   CAD_LegoBrickDefinitionList Bricks;
} Algo_LegoReturn;

PXZ_EXPORTED void Algo_LegoReturn_init(Algo_LegoReturn * str);
PXZ_EXPORTED void Algo_LegoReturn_free(Algo_LegoReturn * str);
typedef struct {
   size_t size;
   Algo_MapType * list;
} Algo_MapTypeList;
...
typedef enum {
 Algo_MaxTransformationType_Similarity = 0,
 Algo_MaxTransformationType_Affine = 1,
 Algo_MaxTransformationType_Homography = 2,
} Algo_MaxTransformationType;
