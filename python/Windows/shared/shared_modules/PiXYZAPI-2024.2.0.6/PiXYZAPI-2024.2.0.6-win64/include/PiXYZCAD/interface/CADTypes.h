...
struct EdgeStyle {
   ...
   CoreI::Double width;
   CoreI::Short pattern;
   EdgeStyle(CoreI::ColorAlpha _color = CoreI::ColorAlpha(), CoreI::Double _width = 1.0, CoreI::Short _pattern = 0) : color(_color), width(_width), pattern(_pattern) {}
};
...
private:
   Type _value;
public:
   LegoType(Type val = Rectangle) : _value(val) {}
   LegoType & operator=(Type val) { _value = val; return *this; }
   LegoType(const CoreI::Int& val) : _value((Type)val) {}
   LegoType & operator=(const CoreI::Int& val) { _value = (Type)val; return *this; }
   Type value() const { return _value; }
   operator Type() const { return _value; }
};
struct LegoBrickDefinition {
   LegoType legoType;
   CoreI::Int length;
   CoreI::Int width;
   CoreI::Boolean fullHeight;
   LegoBrickDefinition(LegoType _legoType = LegoType::Rectangle, CoreI::Int _length = 1, CoreI::Int _width = 1, CoreI::Boolean _fullHeight = true) : legoType(_legoType), length(_length), width(_width), fullHeight(_fullHeight) {}
};

typedef CoreI::List<LegoBrickDefinition> LegoBrickDefinitionList;
...
struct OrientedFace {
   Face face;
   GeomI::Orientation orientation;
   OrientedFace(Face _face = 0, GeomI::Orientation _orientation = true) : face(_face), orientation(_orientation) {}
};
