...
class AnimationPlayer(WindowBase):
    def __init__(self, ui):
        ...
        self.ui.window.callback_manager.addAnimationPlayingStatusChangedCallback(self.onAnimationPlayingStatusChanged)

    def __del__(self):
        self.ui.window.callback_manager.removeAnimationPlayingStatusChangedCallback(self.onAnimationPlayingStatusChanged)

    def draw(self):
        # get copy from library
        animations = self.ui.animationLibrary.getAllCopy()
        if not animations:
            self._currentAnimation = None
        ...
        if self._currentAnimation is not None and self.ui.animationLibrary.animationExists(self._currentAnimation) and pxz.view.isAnimationPlaying(self._currentAnimation):
            pxz.view.applyPlayingAnimations(self.ui.window.viewer.getCurrentTime())
    ...
    def drawTimeSlider(self, animations):
        ...
        if sliderChanged and self.ui.animationLibrary.animationExists(self._currentAnimation):
    ...
    def drawLoopingToggle(self):
        ...
        if isLoopingChanged and self.ui.animationLibrary.animationExists(self._currentAnimation):
    ...
    def drawSpeedCombo(self):
        ...
        if speedChanged and self.ui.animationLibrary.animationExists(self._currentAnimation):
        ...
        if isShowSkeletonChanged:
            if self._showSeleleton:
                try:
                    ...
                    pxz.scene.setParent(self.skeletonOcc, self.ui.window.viewer.hiddenOccurrence)
                except:
                    pass
            else:
