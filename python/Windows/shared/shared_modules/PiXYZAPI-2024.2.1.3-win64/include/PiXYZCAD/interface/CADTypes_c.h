...
typedef enum {
 CAD_BrickType_Rectangle = 0,
 CAD_BrickType_CornerLeft = 1,
 CAD_BrickType_CornerRight = 2,
} CAD_BrickType;
...
PXZ_EXPORTED void CAD_BrickDefinition_init(CAD_BrickDefinition * str);
PXZ_EXPORTED void CAD_BrickDefinition_free(CAD_BrickDefinition * str);
typedef struct {
   size_t size;
   CAD_BrickDefinition * list;
} CAD_BrickDefinitionList;

PXZ_EXPORTED void CAD_BrickDefinitionList_init(CAD_BrickDefinitionList * list, size_t size = 0);
PXZ_EXPORTED void CAD_BrickDefinitionList_free(CAD_BrickDefinitionList * list);
...
PXZ_EXPORTED void CAD_CurveListList_init(CAD_CurveListList * list, size_t size = 0);
PXZ_EXPORTED void CAD_CurveListList_free(CAD_CurveListList * list);
...
typedef enum {
 CAD_ExtrusionBoundaryType_Full = 0,
 CAD_ExtrusionBoundaryType_Hole = 1,
 CAD_ExtrusionBoundaryType_Open = 2,
} CAD_ExtrusionBoundaryType;
...
typedef struct {
   CAD_OrientedEdgeList edges;
   Core_Boolean isOuter;
} CAD_ProfileLoop;

PXZ_EXPORTED void CAD_ProfileLoop_init(CAD_ProfileLoop * str);
PXZ_EXPORTED void CAD_ProfileLoop_free(CAD_ProfileLoop * str);
typedef struct {
   size_t size;
   CAD_ProfileLoop * list;
} CAD_ProfileBase;

PXZ_EXPORTED void CAD_ProfileBase_init(CAD_ProfileBase * list, size_t size = 0);
PXZ_EXPORTED void CAD_ProfileBase_free(CAD_ProfileBase * list);
typedef struct {
   size_t size;
   CAD_ProfileBase * list;
} CAD_ProfileBaseList;

PXZ_EXPORTED void CAD_ProfileBaseList_init(CAD_ProfileBaseList * list, size_t size = 0);
PXZ_EXPORTED void CAD_ProfileBaseList_free(CAD_ProfileBaseList * list);
