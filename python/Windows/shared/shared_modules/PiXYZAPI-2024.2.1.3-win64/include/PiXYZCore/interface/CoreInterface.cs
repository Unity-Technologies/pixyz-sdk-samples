...
namespace UnityEngine.Pixyz.Core {
	...
	internal static GetLicenseErrorReturn ConvertValue(ref GetLicenseErrorReturn_c s) {
		GetLicenseErrorReturn ss = new GetLicenseErrorReturn();
		ss.valid = ConvertValue(s.valid);
		ss.reasons = ConvertValue(s.reasons);
		return ss;
	}

	internal static GetLicenseErrorReturn_c ConvertValue(GetLicenseErrorReturn s, ref GetLicenseErrorReturn_c ss) {
		ss.valid = ConvertValue(s.valid);
		ss.reasons = ConvertValue(s.reasons);
		return ss;
	}
	...
	internal static GetInterfaceLoggerConfigurationReturn_c ConvertValue(GetInterfaceLoggerConfigurationReturn s, ref GetInterfaceLoggerConfigurationReturn_c ss) {
		...
		[DllImport(PiXYZAPI.PiXYZAPI_dll)]
		private static extern GetLicenseErrorReturn_c Core_getLicenseError(IntPtr _pxz_session_);

		/// <summary>
		/// get errors from license check
		/// </summary>
		[System.Runtime.ExceptionServices.HandleProcessCorruptedStateExceptions]
		public Core.GetLicenseErrorReturn GetLicenseError() {
			try {
				var ret = Core_getLicenseError(api.Session);
				System.String err = ConvertValue(Core_getLastError());
				if(!System.String.IsNullOrEmpty(err))
					throw new Exception(err);
				Core.GetLicenseErrorReturn retStruct = new Core.GetLicenseErrorReturn();
				retStruct.valid = (0 != ret.valid);
				retStruct.reasons = ConvertValue(ret.reasons);
				return retStruct;
			}
			catch (System.AccessViolationException e) {
				api.Release();
