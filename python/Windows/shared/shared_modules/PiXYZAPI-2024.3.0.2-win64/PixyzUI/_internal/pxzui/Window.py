...
from pxzui.CallbackManager import CallbackManager
...
class Window:
    ...
    def __init__(self, output, callback_manager, width=1200, height=800):
        ...
        self.drawUI = True  # enable UI by default (state will be saved in config after first close)
        self.resetingSession = False
        self.gpuScenes = [] # list of used GPU Scenes (that needs to be locked for update during processing)
        self.recreateViewer = False

        self.restoreConfig()

        # ensure multiple of 2 viewport size
        self.width = self.width if self.width%2==0 else self.width-1
        self.height = self.height if self.height%2==0 else self.height-1

        #To update first frame
        self.hasEvents = True
        ...
        customTagSuffix = ""
        if pxz.core.getCustomVersionTag() != "Undefined" :
            customTagSuffix = " " + pxz.core.getCustomVersionTag()
        self.glfwWindow = glfw.create_window(self.width, self.height, "Pixyz UI v" + pxz.core.getVersion() + customTagSuffix, None, None)
        if not self.maximized and self.position != None:
        ...
        self.viewer = Viewer(self, self.width, self.height)
        self.viewer.camera.fit(scene.getSelectedOccurrences())  # in case we open it with a model already loaded

        self.powerSaving = False
        self.idleFPS = 60
        self.idleRendererSDKFPS = 1
        self.startFrameTime = glfw.get_time()
        self.startRendererSDKFrameTime = glfw.get_time()
        self.lastxpos = self.lastypos = 0
        self.currentAction = self.InputAction.NONE

        self.callback_manager.addBeforeSessionResetCallback(self.beforeResetSession)
        self.callback_manager.addOnSessionResetCallback(self.afterResetSession)
    ...
    def __del__(self):
        self.callback_manager.removeBeforeSessionResetCallback(self.beforeResetSession)
        self.callback_manager.removeOnSessionResetCallback(self.afterResetSession)
    ...
    def renderLoop(self):
        ...
        while not glfw.window_should_close(self.glfwWindow):
            try:
                ...
                # In powersaving mode : render ImGUI at 60 fps, and only render viewer if an event occurs
                if self.powerSaving and (glfw.get_time()-self.startFrameTime) < (1./self.idleFPS):
                    continue

                renderSDK = not self.powerSaving or self.hasEvents or (glfw.get_time()-self.startRendererSDKFrameTime) > (1./self.idleRendererSDKFPS)
                if self.hasEvents:
                    self.hasEvents = False

                if (glfw.get_time()-self.startRendererSDKFrameTime) > (1./self.idleRendererSDKFPS):
                    self.startRendererSDKFrameTime = glfw.get_time()

                if self.resetingSession:
                    continue
                ...
                if self.recreateViewer:
                    self.recreateViewerAfterResetSession()
                ...
                if monitor and glfw.get_monitor_name(monitor) != glfw.get_monitor_name(self.monitor):
                    ...
                    self.currentFontBold = Fonts.get_bold(monitor_name)
                ...
                gizmoMoved = self.gizmo.draw()
                if gizmoMoved and self.viewer.camera.autoClipping():
                    self.viewer.camera.setProjDirty()
                self.hasEvents = gizmoMoved
    ...
    def whenSizeChanged(self, window, width, height):
        if(width%2 == 1 or height%2 == 1) and glfw.get_window_attrib(self.glfwWindow, glfw.MAXIMIZED)!=1:
            glfw.set_window_size(self.glfwWindow, width if width%2==0 else width-1, height if height%2==0 else height-1)
            return
    ...
    def importFiles(self, paths, useLoadForPxz = False):
        if useLoadForPxz and len(paths) == 1 and paths[0].endswith(".pxz"):
            pxz.core.load(paths[0])
        else:
            pxz.io.importFiles(paths)
        self.viewer.refresh()
        self.viewer.camera.fit([pxz.scene.getRoot()])
    ...
    def cursorPosEvent(self, window, xpos, ypos):
        ...
        if (self.currentAction != self.InputAction.NONE) or glfw.get_mouse_button(self.glfwWindow, 0):
           self.hasEvents = True
        relX = float(xpos - self.lastxpos) / float(self.width)
        relY = float(ypos - self.lastypos) / float(self.height)
        self.lastxpos = xpos
        self.lastypos = ypos
    ...
    def beforeResetSession(self):
        self.resetingSession = True
        self.gpuScenes = [] # clear gpu scene list to avoid storing dead scene id

    def afterResetSession(self):
        # do not recreate viewer here, recreateViewerAfterResetSession will do it in the appropriate thread
        self.recreateViewer = True
        self.resetingSession = False

    def recreateViewerAfterResetSession(self):
        self.viewerState = self.viewer.saveState()
        self.viewer = Viewer(self, self.width, self.height)
        self.viewer.restoreState(self.viewerState)
        self.gizmo = Gizmo(self)
        self.viewer.camera.fit()
        self.ui.afterResetSession()
        self.recreateViewer = False

    def addUsedGPUScene(self, gpuSceneId):
        self.gpuScenes.append(gpuSceneId)

    def removeUsedGPUScene(self, gpuSceneId):
        self.gpuScenes.remove(gpuSceneId)

    def lockGPUScenesUpdate(self):
        pxz.view.lockGPUScenesUpdate(self.gpuScenes)

    def unlockGPUScenesUpdate(self):
        pxz.view.unlockGPUScenesUpdate(self.gpuScenes)
