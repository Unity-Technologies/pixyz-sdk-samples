...
class AnimChannelInfo:
	...
	hasKeyFrames: bool
	def __init__(self, name: str, type: AnimChannelType, mainPropertyBinder: int, hasKeyFrames: bool) -> None: ...
	...
...
class AnimPropertyBinder:
	...
	animation: int
	mainChannel: int
	def __init__(self, occurrence: int, animation: int, mainChannel: int) -> None: ...
	...
...
class AnimationInfo:
	...
	group: str
	length: int
	def __init__(self, name: str, group: str, length: int) -> None: ...
	...
...
class AnnotationDefinition:
	...
	group: int
	shapes: list[int]
	def __init__(self, id: int, name: str, visible: bool, group: int, shapes: list[int]) -> None: ...
	...
...
class Filter:
	...
	name: str
	expr: str
	def __init__(self, id: int, name: str, expr: str) -> None: ...
	...
...
class JointDefinition:
	...
	def __init__(self, id: int, jointComponent: int, isRoot: bool, rootJoint: int, parent: int, children: list[int], matrix: list[list[float]], gloablMatrix: list[list[float]]) -> None: ...
	...
...
class MaterialCount:
	...
	material: int
	count: int
	def __init__(self, material: int, count: int) -> None: ...
	...
...
class PackedTree:
	...
	def __init__(self, occurrences: list[int], parents: list[int], names: list[str], visible: list[core.InheritableBool], materials: list[int], transformIndices: list[int], transformMatrices: list[list[list[float]]], customProperties: list[list[core.StringPair]]) -> None: ...
	...
...
class ProberInfo:
	...
	def __init__(self, occurrence: int, position: geom.Point3) -> None: ...
	...
...
class RayHit:
	...
	def __init__(self, rayParam: float, occurrence: int, triangleIndex: int, triangleParam: geom.Point2) -> None: ...
	...
...
class ResizeByMaximumSizeOptions:
	...
	def __init__(self, TextureSize: int, KeepTextureRatio: bool) -> None: ...
	...

class SceneChangeType(Enum):
	"""
	"""
	PRE_CHILD_INSERTED: int
	CHILD_REMOVED: int
	PROPERTY_CHANGED: int
	COMPLETE_CHANGE: int
	COMPONENT_CHANGED: int
	POST_CHILD_INSERTED: int
	...
...
class VariantDefinition:
	...
	def __init__(self, variant: int, overridedProperties: list[PropertyValue]) -> None: ...
	...
...
class VariantMaterials:
	...
	id: int
	materials: list[int]
	def __init__(self, id: int, materials: list[int]) -> None: ...
	...
...
def createHierarchicalClusters(root: int, childrenCountByNode: int = 2, minFitting: float = -1) -> int:
	...
	Returns:
		root (int): Hierarchical clustering root
	"""
	...
...
def getAABB(occurrences: list[int], precise: bool = False) -> geom.AABB:
	...
	Args:
		...
		precise (bool): If true, will take more time but returns a true AABB. If false, fast but the AABB is approximated and multiple rotation matrices in the tree structure may extend it a lot
...
def getActiveMaterials(occurrences: list[int]) -> list[int]:
	...
	Args:
		occurrences (list[int]): The occurrences to retrieve active material on

	Returns:
		materials (list[int]): The active material for each given occurrence
	"""
	...
...
def getBrepShape(part: int) -> int:
	...
	Returns:
		brepShape (int): The brep shape of the part
	"""
	...
...
def getGlobalMatrices(occurrences: list[int]) -> list[list[list[float]]]:
	...
	Args:
		occurrences (list[int]): Nodes to get the local matrix

	Returns:
		matrices (list[list[list[float]]]): The global matrix for each given node
	"""
	...
...
def getLocalMatrices(occurrences: list[int]) -> list[list[list[float]]]:
	...
	Returns:
		matrices (list[list[list[float]]]): The local matrix for each given node
	"""
	...
...
def getMBB(occurrences: list[int], forcedAxis: geom.Point3 = geom.Vector3(0,0,0)) -> geom.OBB:
	...
	Args:
		occurrences (list[int]): List of occurrences to retrieve the MBB
		forcedAxis (geom.Point3): Forced axis of the mbb
...
def getOBB(occurrences: list[int]) -> geom.OBB:
	...
	Args:
		occurrences (list[int]): List of occurrences to retrieve the OBB
...
def getSubTreeStats(roots: list[int]) -> dict:
	...
	Returns:
		...
		patchesCount (int): Number of patches in the sub-tree (instances are counted multiples times)
	"""
	...
...
def getVolume(occurrence: int) -> float:
	...
	Returns:
		volume (float): The computed volume
	"""
	...
...
def getVolumes(iRoots: list[int]) -> list[float]:
	...
	Args:
		iRoots (list[int]): List of roots

	Returns:
		volumes (list[float]): The computed volumes
	"""
	...
...
def isAncestorOf(maybeAncestor: int, occurrence: int) -> bool:
	...
	Args:
		maybeAncestor (int): The potential ancestor of the occurrence
		occurrence (int): The occurrence to test
...
def keepAncestors(occurrences: list[int]) -> list[int]:
	...
	Args:
		occurrences (list[int]): The list that shall be filtered.

	Returns:
		filteredList (list[int]): The list without descendants of members.
	"""
	...
...
def setSubpartMaterials(occurrence: int, materials: list[int], startIndex: int = 0) -> None:
	...
	Args:
		occurrence (int): The Occurrence on which to apply the materials
		materials (list[int]): Materials to set
		startIndex (int): The subpart start index
	"""
	...
...
def getTessellationParameters(part: int) -> dict:
	...
	Args:
		part (int): The tessellated part

	Returns:
		maxSag (float): The MaxSag value in mm
		maxLength (float): The maximum length in mm
		maxAngle (float): 
		createNormals (bool): Whether normals were created
		uvGenerationMode (UVGenerationMode): The uv generation mode
		uvChannel (int): The UV channel
		uvPadding (float): The UV padding
		createTangents (bool): Whether tangents were created
		createFreeEdges (bool): Whether free edges were created
	"""
	...
...
def mergePartOccurrencesByFinalAssemblies(roots: list[int] = list[int](), mergeHiddenPartsMode: MergeHiddenPartsMode = 0, CollapseToParent: bool = True) -> None:
...
def setLocalMatrices(occurrencesIds: list[int], matrices: list[list[list[float]]], batchSize: int) -> None:
	...
	Args:
		occurrencesIds (list[int]): List of occurrences on which to set the local matrices
		matrices (list[list[list[float]]]): The occurrences' new local matrices
		batchSize (int): Number of matrices to be set per thread.
	"""
	...
...
def createAnnotationFromDefinition(definition: AnnotationDefinition) -> int:
	...
	Returns:
		metadata (int): Annotation created from definition
	"""
	...
...
def createOccurrenceFromAnnotation(annotation: int, convertVisibility: bool = False) -> int:
	...
	Args:
		annotation (int): Annotation that shall be converted to an occurrence with mesh geometry
		convertVisibility (bool): If True, the visibility will be converted (useful to select a specific view/variant), else all occurrences will be visible

	Returns:
		newOccurrence (int): Annotation as occurrence with mesh geometry
	"""
	...
...
def getAnnotationListAABB(annotationList: list[int]) -> geom.AABB:
	...
	Args:
		annotationList (list[int]): 

	Returns:
		aabb (geom.AABB): The axis aligned bounding box of the given annotation list
	"""
	...
...
def getPrototypes(occurrences: list[int]) -> list[int]:
	...
	Args:
		occurrences (list[int]): The occurrence

	Returns:
		prototypes (list[int]): The prototype of each occurrence
	"""
	...
...
def setPrototypes(occurrences: list[int], prototypes: list[int]) -> None:
	...
	Args:
		occurrences (list[int]): The occurrences
		prototypes (list[int]): The prototypes
	"""
	...
...
def getSelectedPolygonCount() -> int:
	"""Returns the number of selected polygons

	Returns:
		polygonCount (int): Number of selected Polygons
	"""
	...
...
def invertSelect(occurrence: list[int]) -> None:
	"""Invert occurrences to selection

	Args:
		occurrence (list[int]): Occurrences to invert from the selection
	"""
	...
...
# Callbacks
def addComponentChangedCallback(callback: Callable[[ComponentChangeType, int, int], None], userdata):
	"""
	"""
	...

def removeComponentChangedCallback(id: int):
	"""Remove callback added with scene.addComponentChangedCallback
	"""
	...
