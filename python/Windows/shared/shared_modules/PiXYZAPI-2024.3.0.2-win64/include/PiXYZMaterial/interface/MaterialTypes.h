...
struct Texture {
   ...
   explicit Texture(Image _image = 0, CoreI::Int _channel = 0, GeomI::Point2 _offset = GeomI::Point2(), GeomI::Point2 _tilling = GeomI::Point2()) : image(_image), channel(_channel), offset(_offset), tilling(_tilling) {}
};
...
struct ColorMaterialInfos {
   ...
   explicit ColorMaterialInfos(CoreI::String _name = "", CoreI::ColorAlpha _color = CoreI::ColorAlpha()) : name(_name), color(_color) {}
};
...
struct MaterialDefinition {
   ...
   explicit MaterialDefinition(CoreI::String _name = "", Material _id = 0, ColorOrTexture _albedo = ColorOrTexture(), ColorOrTexture _normal = ColorOrTexture(), CoeffOrTexture _metallic = CoeffOrTexture(), CoeffOrTexture _roughness = CoeffOrTexture(), CoeffOrTexture _ao = CoeffOrTexture(), CoeffOrTexture _opacity = CoeffOrTexture(), ColorOrTexture _emissive = ColorOrTexture()) : name(_name), id(_id), albedo(_albedo), normal(_normal), metallic(_metallic), roughness(_roughness), ao(_ao), opacity(_opacity), emissive(_emissive) {}
};

typedef CoreI::List<MaterialDefinition> MaterialDefinitionList;

struct MaterialFromMapsReturn {
   //! Number of newly imported textures
   CoreI::Int importedTextureCount;
   //! Number of newly created materials
   CoreI::Int createdMaterialCount;
   CoreI::Int updatedMaterialCount;
   explicit MaterialFromMapsReturn(CoreI::Int _importedTextureCount = 0, CoreI::Int _createdMaterialCount = 0, CoreI::Int _updatedMaterialCount = 0) : importedTextureCount(_importedTextureCount), createdMaterialCount(_createdMaterialCount), updatedMaterialCount(_updatedMaterialCount) {}
};
...
private:
   Type _value;
public:
   ResizeFilterMethod(Type val = DEFAULT) : _value(val) {}
   ResizeFilterMethod & operator=(Type val) { _value = val; return *this; }
   ResizeFilterMethod(const CoreI::Int& val) : _value((Type)val) {}
   ResizeFilterMethod & operator=(const CoreI::Int& val) { _value = (Type)val; return *this; }
   Type value() const { return _value; }
   operator Type() const { return _value; }
};
...
typedef struct {
   CoreI::IntList widths;
   CoreI::IntList heights;
