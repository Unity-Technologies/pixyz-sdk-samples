...
struct AnimChannelInfo {
   ...
   explicit AnimChannelInfo(CoreI::String _name = "", AnimChannelType _type = AnimChannelType::SIMPLE, CoreI::Ident _mainPropertyBinder = 0, CoreI::Bool _hasKeyFrames = false) : name(_name), type(_type), mainPropertyBinder(_mainPropertyBinder), hasKeyFrames(_hasKeyFrames) {}
};
...
struct AnimPropertyBinder {
   ...
   explicit AnimPropertyBinder(Occurrence _occurrence = 0, Animation _animation = 0, AnimChannel _mainChannel = 0) : occurrence(_occurrence), animation(_animation), mainChannel(_mainChannel) {}
};
...
struct AnimationInfo {
   ...
   explicit AnimationInfo(CoreI::String _name = "", CoreI::String _group = "", AnimationTime _length = 0) : name(_name), group(_group), length(_length) {}
};
...
struct AnnotationDefinition {
   ...
   explicit AnnotationDefinition(Annotation _id = 0, CoreI::String _name = "", CoreI::Boolean _visible = false, AnnotationGroup _group = 0, TessellatedShapeList _shapes = TessellatedShapeList()) : id(_id), name(_name), visible(_visible), group(_group), shapes(_shapes) {}
};
...
private:
   Type _value;
public:
   ComponentChangeType(Type val = COMPONENT_ADDED) : _value(val) {}
   ComponentChangeType & operator=(Type val) { _value = val; return *this; }
   ComponentChangeType(const CoreI::Int& val) : _value((Type)val) {}
   ComponentChangeType & operator=(const CoreI::Int& val) { _value = (Type)val; return *this; }
   Type value() const { return _value; }
   operator Type() const { return _value; }
};
...
struct Filter {
   ...
   explicit Filter(CoreI::Ident _id = 0, CoreI::String _name = "", FilterExpression _expr = "") : id(_id), name(_name), expr(_expr) {}
};
...
struct MaterialCount {
   ...
   CoreI::Int count;
   explicit MaterialCount(MaterialI::Material _material = 0, CoreI::Int _count = -1) : material(_material), count(_count) {}
};
...
struct PropertyValue {
   ...
   CoreI::String value;
   explicit PropertyValue(CoreI::String _name = "", CoreI::String _value = "") : name(_name), value(_value) {}
};
...
struct ProberInfo {
   ...
   explicit ProberInfo(Occurrence _occurrence = 0, GeomI::Point3 _position = GeomI::Point3()) : occurrence(_occurrence), position(_position) {}
};
...
struct RayHit {
   ...
   explicit RayHit(CoreI::Double _rayParam = -1, Occurrence _occurrence = 0, CoreI::Int _triangleIndex = -1, GeomI::Point2 _triangleParam = GeomI::Point2()) : rayParam(_rayParam), occurrence(_occurrence), triangleIndex(_triangleIndex), triangleParam(_triangleParam) {}
};
...
struct ResizeByMaximumSizeOptions {
   ...
   explicit ResizeByMaximumSizeOptions(CoreI::Int _TextureSize = 4096, CoreI::Boolean _KeepTextureRatio = true) : TextureSize(_TextureSize), KeepTextureRatio(_KeepTextureRatio) {}
};
...
private:
   Type _value;
public:
   SceneChangeType(Type val = PRE_CHILD_INSERTED) : _value(val) {}
...
private:
   Type _value;
public:
   UVGenerationMode(Type val = NO_UV) : _value(val) {}
...
struct VariantDefinition {
   ...
   explicit VariantDefinition(Variant _variant = 0, PropertyValueList _overridedProperties = PropertyValueList()) : variant(_variant), overridedProperties(_overridedProperties) {}
};
...
struct VariantMaterials {
   ...
   //! the material list
   MaterialI::MaterialList materials;
   explicit VariantMaterials(Variant _id = 0, MaterialI::MaterialList _materials = MaterialI::MaterialList()) : id(_id), materials(_materials) {}
};
...
typedef struct {
   CoreI::Double maxSag;
   CoreI::Double maxLength;
   CoreI::Double maxAngle;
   CoreI::Bool createNormals;
   UVGenerationMode uvGenerationMode;
   CoreI::Int uvChannel;
   CoreI::Double uvPadding;
   CoreI::Bool createTangents;
   CoreI::Bool createFreeEdges;
